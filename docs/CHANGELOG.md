...
- **待发布 — 顶部栏布局优化（#48）**
  ...
  - 顶部栏右侧「关于 / 设置 / 账号 / 同步日志 / 立即同步」按钮组在窗口过窄时不再换行（`flex-wrap: nowrap`），过窄时触发整窗横向滚动。
  - 移除顶栏最左侧的 "TaskBoard" 品牌文字，释放空间、减少视觉冗余。
  - 顶部栏右侧 5 个按钮内联 SVG 图标，窗口过窄（≤1100px）时仅显示图标；「同步日志」改为 i18n key。
  - 设置 / 关于 / 账号 / 同步日志四个弹窗统一收敛到互斥的 `activeModal` 状态，同一时刻仅显示一个弹窗（修复新增按钮后弹窗叠加，根因同 #26）。
  - 暂隐藏「全部账号」视图模式，仅保留单账号模式。
  - 弹窗容器 `.modal` 高度随视口收敛（`max-height` + `overflow-y: auto`），修复矮窗口下标题栏 / 底部被截断。
  - 弹窗遮罩提升 `z-index: 20`（高于顶栏/搜索栏的 5），修复矮窗口下弹窗顶部被搜索栏压住。
  - 知识库文档：[`docs/issue-48-topbar-layout.md`](./issue-48-topbar-layout.md)
